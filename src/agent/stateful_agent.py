...
TOOLS = [get_weather, list_flights, list_hotels, convert_currency, book_flight, book_hotel]
...
def assistant(gs: GraphState):
    ...
    sys = SystemMessage(
        content=(STATEFUL_SYSTEM_PROMPT + "\n" + json.dumps(st, ensure_ascii=False) + "\n" + CONSTRAINTS)
    )
    ...
    if isinstance(last_msg, AIMessage) and last_msg.tool_calls:
        for tc in last_msg.tool_calls:
            ...
            if tool_name == "get_weather":
                ...
                if city == "Bangkok":
                    new_state["weather_checked"][city] = "Hot, humid, lots of rain"
                elif city == "Dubai":
                    new_state["weather_checked"][city] = "Very hot, dry, no rain"
                elif city == "Reykjavik":
                    new_state["weather_checked"][city] = "Very cold with snow, little rain"
                ...
                if isinstance(dest, str) and dest in CITY_CODE_BY_NAME.values() and not new_state.get("selected_city"):
                ...
                if isinstance(city, str) and city in CITY_CODE_BY_NAME.values() and not new_state.get("selected_city"):
...
def post_tool_update(gs: GraphState):
    ...
    def _parse(c):
        if isinstance(c, (dict, list)): return c
    ...
    for tm in tail:
        ...
        if tm.name == "book_flight" and _is_correct_span_for_tool("book_flight", parsed):
            new_state["flight_booking"] = parsed
        
        elif tm.name == "book_hotel" and _is_correct_span_for_tool("book_hotel", parsed):
...
initial_state = {
    "weather_checked": {"Bangkok": "", "Dubai": "", "Reykjavik": ""},
    "selected_city": None,
    "flights_01_08": [],
    "hotels_01_08": [],
    "flights_02_09": [],
...
def run_stateful_trial(user_prompt: str | None = None, recursion_limit: int = 40, live: bool = False):
    """Run the stateful agent once; if live, stream node updates like baseline agent.

    Returns the final GraphState (contains messages + state).
    """
    # Fresh copy of initial state each run
    gs: GraphState = {
    ...
    if not live:
        return graph.invoke(gs, {"recursion_limit": recursion_limit})

    final: GraphState | None = None
    for update in graph.stream(gs, {"recursion_limit": recursion_limit}, stream_mode="updates"):
        node_name = next(iter(update.keys()))
        node_payload = update[node_name]

        # print last message when a node yields messages
        if isinstance(node_payload, dict) and node_payload.get("messages"):
        ...
        # Track latest payload containing state/messages so we can return it
        if isinstance(node_payload, dict) and node_payload.get("messages"):
            final = node_payload  # type: ignore

    if final is None:  # Fallback
        final = graph.invoke(gs, {"recursion_limit": recursion_limit})  # type: ignore
